...
namespace DebugUI
{
	...
	void InitDataInOut()
	{
		...
		WIN_DATA_INOUT.AddItem(Item::CreateValue("Terrain", Item::Kind::Bool, true, true));			// ライトオブジェクトを含むか
		WIN_DATA_INOUT.AddItem(Item::CreateValue("Enemy", Item::Kind::Bool, true, true));			// ライトオブジェクトを含むか
	...
	void InitObjectTypeList()
	{
		...
		WIN_OBJ_TYPE_LIST.AddItem(Item::CreateGroup("Item"));
		...
		WIN_OBJ_TYPE_LIST["Item"].AddGroupItem(Item::CreateList("ItemList", Clickfunc, false, false, false, 5));
	...
	void CreateObjectTypeList()
	{
		...
		for (auto& objectType : objectTypeMap)
		{
			...
			// アイテム
			else if (category == ObjectTypeRegistry::ObjectCategoryType::OCT_ITEM)
				WIN_OBJ_TYPE_LIST["Item"]["ItemList"].AddListItem(objectType.first.c_str());
			// カメラ
