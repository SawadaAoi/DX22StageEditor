...
=========================================== */
template<typename T>
inline T* UIObjectBase::AddComponent()
{
	// 基底クラスがUIComponentBaseでない場合はエラーを出力
	if (std::is_base_of<UIComponentBase, T>() == false)	return nullptr;
	...
	{
		// 更新順序が小さい順に挿入
		if (nOrder < static_cast<UIComponentBase*>((*it).get())->GetUpdateOrder())
		{
...
=========================================== */
template<typename T>
inline T* UIObjectBase::GetComponent()
{
	// 基底クラスがUIComponentBaseでない場合はエラーを出力
	if (std::is_base_of<UIComponentBase, T>() == false)	return nullptr;
...
=========================================== */
template<typename T>
inline void UIObjectBase::RemoveComponent()
{
	// 基底クラスがUIComponentBaseでない場合はエラーを出力
	if (std::is_base_of<UIComponentBase, T>() == false)	return nullptr;
